...
import { SizeContext } from "../utilities/size-observe";
import useAnimation from "../utilities/use-animation";
...
const SliderContainer: React.FC<Props> = ({ children, initialOffsetX, contentWidth, className }) => {
    ...
    useAnimation(enabled, useCallback(() => {
        const { current: elContainer } = refContainer;
        const { current: elContent } = refContent;
        if (elContainer && elContent) {
            refScrollX.current += 0.6
...
export const SliderItem: React.FC<ItemProps> = ({ children, width }) => {
    return <div className="inline-flex justify0center items-center mx-4" style={{ width }}>
        {children}
    </div>
}
